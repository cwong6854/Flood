...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        ...
        if (place.row < _height - 1) {
            Place down = pl(place.row + 1, place.col);
            action.accept(down);
        }
        if (place.col > 0) {
    ...
    boolean[] colorsPresent() {
        ...
        int x = 0;
        while (x < _ncolors) {
            for (int h = 0; h < _cells.length; h += 1) {
                for (int w = 0; w < _cells[h].length; w += 1) {
                    if (_cells[h][w] == x) {
                        colors[x] = true;
                    }
                }
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        Consumer<Place> reachadd = (p) -> {
            findRegion(p, color, result);
        };
        if (result.contains(start) || get(start) != color) {
            return result;
        }
        result.add(start);
        forNeighbors(start, reachadd);
        return result;
    }
    ...
    void setActiveRegionColor(int newColor) {
        for (Place region : _active) {
            _cells[region.row][region.col] = newColor;
            _active = sameColorRegion(region);
        }
        _current += 1;
        _lastHistory += 1;
        _history.add(new GameState());
    ...
    Set<Place> adjacentCells(int color) {
        HashSet cellsactive = new HashSet<>();
        for (Place adjacent : _active) {
            forNeighbors(adjacent, (p) -> findRegion(p, color, cellsactive));
        }
        return cellsactive;
    }
